...
- 当前分支：`main`
- 当前阶段：`1.9.2 三层准备度内核定型版` 已发布并完成生产复查
- 工作区状态：1.9.2 三层准备度内核、目标管理承接和生产限流护栏已完成发布验证，下一步进入 1.9.3 数据治理闭环
- 文档索引：`docs/README.md`
...
- 生产部署：已部署提交 `bd5fe75`，`study-planner-api` online，`/api/version` 返回 `1.9.0`，2026-05-05。
- 生产限流复查：`/api/health` 和 `/api/version` 不进入业务限流，登录限流为 `20`，`/api/children` 业务限流为 `2000`，连续 5 次 `scripts/check-production.sh` 未出现 `429`，2026-05-05。

历史发布记录、旧路线图、旧检查记录和旧 ToDo 已按类型归档到 `docs/releases/`、`docs/roadmaps/`、`docs/checks/`、`docs/todos/`。

## 建议下一步

1. 进入 `1.9.3 数据治理闭环版`，把数据体检从发现问题升级为定位、修复、复查闭环。
2. `1.9.4` 再进入规则诊断增强，完善置信度、Velocity、余力指数和跨学科交叉验证。
