...
const localChildSettingsKey = 'quxueban_child_settings_v1';
...
function loadLocalChildSettings(childId?: number | null): LocalChildSettings {
  const defaults = getDefaultLocalChildSettings();
  if (!childId) return defaults;

  try {
    const raw = localStorage.getItem(localChildSettingsKey);
    const parsed = raw ? JSON.parse(raw) : {};
    return { ...defaults, ...(parsed[String(childId)] || {}) };
  } catch {
    return defaults;
  }
}

function saveLocalChildSettings(childId: number, settings: LocalChildSettings) {
  try {
    const raw = localStorage.getItem(localChildSettingsKey);
    const parsed = raw ? JSON.parse(raw) : {};
    localStorage.setItem(localChildSettingsKey, JSON.stringify({ ...parsed, [String(childId)]: settings }));
  } catch {
    // Local settings are best-effort until these fields are backed by APIs.
  }
}
...
export default function ChildrenManagement({
  ...
  const persistLocalSettings = (settings = localSettings, successMessage = '配置已保存') => {
    if (!selectedChild) return;
    saveLocalChildSettings(selectedChild.id, settings);
  ...
  const handleOpenDetail = (child: Child) => {
    ...
    setLocalSettings(loadLocalChildSettings(child.id));
    setNewCustomTag('');
    setNewInterestTag('');
  ...
  const handleUpdate = () => {
    ...
    saveLocalChildSettings(selectedChild.id, localSettings);
  };
                ...
                <section className="h-full overflow-y-auto rounded-xl border border-border/70 bg-white p-3 shadow-sm">
                  ...
                  <div className="rounded-xl border border-blue-100 bg-blue-50/50 p-3">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <Badge variant="outline" className="border-blue-200 bg-white text-blue-700">本地保存</Badge>
                    </div>
                ...
                <section className="h-full overflow-y-auto rounded-xl border border-border/70 bg-white p-3 shadow-sm">
                  ...
                  <div className="rounded-xl border border-violet-100 bg-violet-50/45 p-3">
                    <div className="mb-3 flex items-center justify-between gap-3">
                      ...
                      <Badge variant="outline" className="border-violet-200 bg-white text-violet-700">待接入后端</Badge>
                    </div>
                    <div className="grid gap-3 md:grid-cols-3">
                      <div className="space-y-2">
                        <Label>学习阶段</Label>
                ...
                <section className="h-full overflow-y-auto rounded-xl border border-border/70 bg-white p-3 shadow-sm">
                  ...
                  <div className="rounded-xl border border-emerald-100 bg-emerald-50/45 p-3">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <Badge variant="outline" className="border-emerald-200 bg-white text-emerald-700">本地保存</Badge>
                    </div>
                    <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <div className="flex justify-end">
                    <Button className="bg-violet-600 hover:bg-violet-700" onClick={() => persistLocalSettings(localSettings, '兴趣设置已保存')}>保存更改</Button>
                  </div>
                  </div>
                </section>
              )}
