...
export default function TestParentLayout() {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [sidebarCollapsed] = useState(true);
  const { user, logout, isAuthenticated, isInitializing } = useAuth();
  const { children: childrenList, selectedChild, selectChild, isLoading } = useSelectedChild();
  ...
  useEffect(() => {
    if (!isInitializing && !isAuthenticated) {
      console.log('[TestParentLayout] Redirecting to login page');
      navigate('/login', { replace: true, state: { from: location } });
    } else if (!isInitializing && isAuthenticated && user?.role !== 'parent') {
      // 孩子用户访问家长页面，重定向到孩子首页
      console.log('[TestParentLayout] Redirecting to child dashboard');
  ...
  if (isInitializing || isLoading) {
    console.log('[TestParentLayout] Initializing...');
    return (
  ...
  if (!isAuthenticated) {
    console.log('[TestParentLayout] Not authenticated, rendering loading state');
    return (
      <div className="min-h-screen bg-[#F5F5F7] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-primary flex items-center justify-center text-white font-bold text-xl shadow-sm animate-pulse">
  ...
  console.log('[TestParentLayout] Rendering with user:', user?.name, 'familyId:', user?.familyId);
      ...
      <div className="p-3 border-t border-gray-200/50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-gray-900 text-sm">家长</p>
